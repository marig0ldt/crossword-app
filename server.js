...
io.on('connection', (socket) => {
  ...
  socket.on('fill_cell', async ({ cellIndex, letter }) => {
    ...
    if (correct) await db.run('UPDATE players SET score = score + 10, cells_filled = cells_filled + 1 WHERE id = ?', [playerId]);
  ...
  socket.on('request_start_game', () => { if (socket.data?.roomId) startGameForRoom(socket.data.roomId); });

  socket.on('chat_message', (data) => {
    if (socket.data?.roomId) io.to(socket.data.roomId).emit('chat_message', { username: socket.data.username, message: data.message.slice(0, 200) });
  });

  socket.on('disconnect', async () => {
    if (socket.data?.playerId) await db.run('UPDATE players SET socket_id = NULL WHERE id = ?', [socket.data.playerId]);
  });
});
